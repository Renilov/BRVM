...
def maj_donnees_brvm():
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()

    # Liste des sociétés BRVM avec leurs vrais cours et dividendes
    donnees = [
    ...
    for ticker, nom, secteur, cours, div, detach, paie in donnees:
    ...
    conn.commit()
    conn.close()
    print("✅ La base de données 'brvm_invest.db' a été mise à jour avec succès !")
